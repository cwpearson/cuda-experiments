...
static void gpu_gpu_bw(const Device &dst, const Device &src, const size_t count)
{
  ...
  for (size_t i = 0; i < numIters; ++i)
  {
    ...
    gpu_write<<<gridDim, blockDim>>>(dstPtr, pageSize, count);
    RT_CHECK(cudaDeviceSynchronize());
...
int main(void)
{
  ...
  auto counts = Sequence::geometric(2048, 2ul * 1024ul * 1024ul * 1024ul, 2) |
                Sequence::geometric(2048, 2ul * 1024ul * 1024ul * 1024ul, 1.5);
