...
static void pinned_bw(const Device &dst, const Device &src, const size_t count)
{
  ...
  RT_CHECK(cudaHostAlloc(&hostPtr, count, cudaHostAllocWriteCombined));
...
int main(void)
{
  ...
  for (const auto cpu : cpus)
  {
    ...
    {
      printf(",%s to %s (wc)", cpu.name().c_str(), gpu.name().c_str());
    }
  }
  //gpu->cpu
  for (const auto cpu : cpus)
  {
    for (const auto gpu : gpus)
    {
      printf(",%s to %s (wc)", gpu.name().c_str(), cpu.name().c_str());
    }
  }
